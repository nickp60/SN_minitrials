...
transformed parameters {
  ...
  //vector[nrows] colif_hat; // the *real* pathogen load, without measurement error
 ...
 //   colif_hat[i] = ((temp_effect_m[Temp[i]] + recipe_effect_m[Recipe[i]] +  ol_effect_m[OL[i]])  * Time[i]) + (recipe_effect_b[Recipe[i]] + temp_effect_b[Temp[i]] + ol_effect_b[OL[i]]);
...
model {
  ...
  //Coliforms ~ normal(colif_hat, sigma);
