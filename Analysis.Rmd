...
```{r}
(meth1 <- ggplot(sn, aes(x=Time, y=Methane, color=Recipe)) +
  geom_point()  + geom_smooth(method = lm) + 
  facet_grid(~Temp )  +
  scale_color_brewer(palette = "Set1") +
  labs(caption=fn("Effect of Temperature and Recipe"), color=""))
(meth2 <- ggplot(sn, aes(x=Time, y=Methane, color=Recipe)) +
  geom_point()  + geom_smooth(method = lm) + 
  facet_grid(Temp~OL )  +
  scale_color_brewer(palette = "Set1") +
  labs(caption=fn("Effect of Temperature and Recipe by OL"), color=""))
(meth3 <- ggplot(sn %>% filter(Time!=0), aes(x=Time, y=Methane, color=as.factor(Temp))) +
  geom_point()  + geom_smooth(method = lm) + 
  facet_grid(Recipe~OL )  +
  scale_color_viridis_d() +
  labs(caption=fn("Effect of Temperature and Recipe by OL, excluding Day 0"), color=""))
(meth4 <- ggplot(sn, aes(x=Time, y=Methane, color=as.factor(Temp))) +
  geom_point()  + geom_smooth(method = lm) + 
  facet_wrap(~Recipe )  +
  labs(caption=fn("Effect of Temperature by Recipe"), color=""))
ggsave(paste0("methane_", 1, ".png"), plot = meth1, device = "png", width = 6, height = 6)
ggsave(paste0("methane_", 2, ".png"), plot = meth2, device = "png", width = 6, height = 6)
ggsave(paste0("methane_", 3, ".png"), plot = meth3, device = "png", width = 6, height = 6)
ggsave(paste0("methane_", 4, ".png"), plot = meth4, device = "png", width = 6, height = 6)
  ...
  error="sem")
{
  ...
  newdf$FIB <- as.factor(newdf$FIB)
  levels(newdf$FIB) <-   c("textstyle('Coliforms')","italic('E. coli')", "textstyle('Enterococci')")
  if (error == "sem"){
...
#        color="black",
      ...
      facet_grid(Recipe ~ FIB, scales="free_y", labeller=label_parsed) + 
...
ggsave(p1 + labs(title=NULL, caption=NULL, subtitle=NULL), filename = "./removalplot.png", device = "png", width = 6, height = 5)
ggsave(p1 + labs(title=NULL, caption=NULL, subtitle=NULL), filename = "./removalplot.pdf", device = "pdf", width = 7, height = 6)
```
